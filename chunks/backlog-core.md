...
are about to work on to **In Progress** (`backlog task edit <id> -s "In Progress"`). This
status transition is not optional — a task silently worked while still "To Do" is invisible
to the board.
...
**An open-ended trial gets an OWNING TASK, a hard timebox, and a "verdict recorded" AC.**
Play, spikes, prototypes and "see how it feels" explorations are still work, and without a row
they end by drifting rather than by deciding. Give each one a task whose description carries a
**hard timebox** ("3 days of play") and whose AC is the verdict itself — *recorded by day N
whatever the state*, with an explicit exit for the inconclusive case (an inconclusive verdict
converts to registered options + leanings, which is a result, not a failure). A soft intention
to "wrap this up soon" is not an exit gate; the AC is.
...
**No internal date windows in task descriptions.** A task row is not a calendar. Dates written
into a description ("ship by Oct 5", "the Aug-12 runway") go stale silently the moment the plan
moves, and nothing on the board flags them — a whole re-dated task set can be voided by one
postponement decision while every row still reads as current. Keep schedule in the plan doc or the
ADR that owns it, and let the row carry the work.
...
never open is not enough. **A finding never goes homeless:** if it constrains no
existing row, it still needs an owner — a new task, an ADR, or a named note — before the producing
task closes. "Interesting, but nothing to attach it to" is how a real result gets lost.
