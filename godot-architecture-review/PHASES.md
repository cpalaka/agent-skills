...
`codex-skills/godot-architecture-review` adapter says what to do with that token.
...
opinionated, deduped glossary; write `CONTEXT.md`. This seeds every later phase — and all future
development — with a pinned vocabulary. Phase 0 produces *vocabulary only*; the whole-game map
is Phase 1.
...
1. **Orient on `docs/adr/` first** — ADRs are scope-fences; a dive must not contradict one without
   surfacing the conflict for an explicit amend/supersede decision *before* the grill locks the
   thread. Sweep for candidate-adjacent ADRs, not just exact-name matches — an amendment may be
   owed to an ADR about a neighboring seam.
