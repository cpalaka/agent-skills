...
which the alias rule (measured 2026-07-24) forbids: an alias can lag a release and keep serving the
prior generation while every rule still reads correct.
...
as an alias for `critic` (live launch snippets still use it); an unrecognized posture falls back to
`none` **with a logged warning**, never silently.
...
Method: one finder lens (`engine-gotchas`), one fixed 8-file code diff in a game project, N=3
workhorse vs N=3 scarce. Byte-identical prompt built as a single shared const, identical `xhigh`
effort, both arms in one `parallel()` so they share an execution window; the only differing input is
the model ID. 6 agents, 0 dropped, 0 empty.
...
The one clean asymmetry runs *against* the rung. The defect "a new stray guard runs before, and
therefore disables, an export script's own stale-backup recovery" was found 3/3 by workhorse and
...
This measurement replaces an earlier design that could not have worked: the only prior
adversarial-review run with per-finding results reviewed a spec whose reviewed bytes were never
committed, and whose committed version has the review's own findings written into it — re-running
finders against it would have scored them against the answer key.
...
Scope note: workflow scripts that live in consuming projects rather than in this repo are not swept
by this decision. Several still carry alias pins and the boolean-era posture; they are recorded as
known-stale, because one of them is deliberately all-scarce and needs a posture decision, not a
mechanical fix.
