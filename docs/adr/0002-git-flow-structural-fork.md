...
**Consequences (the non-obvious coupling):** **no-SHA-in-notes is a consequence of squash** — the
squash commit does not exist when the task is marked Done on the branch, so its SHA can't be
recorded; `--no-ff` produces a real merge SHA at marking time, so it *requires* the hash.
Therefore the SHA rule lives in the git-flow variant files, **not** in `backlog-core` (which
stays merge-agnostic and defers to "your git-flow variant"). Emitting the squash no-SHA rule into
a `--no-ff` project — or vice versa — would be a bug.
