...
- **Effort `high` on every seat**, in the definition's frontmatter or a workflow stage's `effort`;
  the Agent tool pins only `model`. No higher effort. Never pass `model` on a seat dispatch: it
  overrides the definition's pin.
