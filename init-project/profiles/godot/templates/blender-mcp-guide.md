...
One MCP server bridges to a running Blender instance over **TCP `localhost:9876`** (not the godot-mcp port 6550). Claude Code tool prefix `mcp__blender-mcp__`; Codex names the same server `blender`. Blender must be running with the MCP add-on enabled and connected. Single-client bridge.
