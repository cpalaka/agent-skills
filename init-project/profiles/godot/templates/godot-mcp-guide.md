...
- **godot-ai** (`hi-godot/godot-ai` v4.0.2, vendored at `addons/godot_ai/`, **tracked in git** — it self-updates in-editor, gotcha #116; added 2026-05-30, last bumped 2026-09-17) — construction-grade editor MCP. **v4 needs Godot 4.7+ (within the 4.x line) and `uv` on PATH**; the plugin auto-starts its Python server and connects over **WebSocket**. Register a client through the **Godot AI** dock's **Configure** button, which writes a generated `godot-ai attach` stdio entry carrying the matching version, ports and tool-domain filters — **do not hand-write a bare HTTP MCP entry**: `http://127.0.0.1:8000/mcp` was the v3 shape, and v3→v4 is a migration (`docs/v4-migration.md` in the addon repo). Catalog at 4.0.2: **41 tools across 27 domains**, read from `addons/godot_ai/tool_catalog.gd` on 2026-09-17. The surface listed here was diffed at **v3.2.4** against the 3.1.3 tree on 2026-09-02 and is kept as that version's record (new since 3.1.3: `gridmap_manage`, `csg_manage`, `custom_manage` — the last invokes tools other addons register via the 3.2.0 custom-tool registry; nothing here registers one): `scene_*`/`node_*`/`script_*`, `signal_manage`, `material`/`particle`/`camera`/`animation`/`ui_manage`, `project_run`, `editor_screenshot`, `logs_read`, `game_manage` (v2.7.x: **raw** key/mouse/gamepad input injection — `input_key`/`input_mouse`/`input_gamepad`, which drive any *mapped* action — plus runtime node-state read `get_node_info`/`get_scene_tree` and named-action-state read `input_state`; verified live on 2.7.5 / Godot 4.7). Enabled as a project plugin. Since 3.2.0 the dock's client registration scope is an EditorSettings enum (`user` default → the stdio entry lives in the host's user-scope config, not `.mcp.json`; see the contract, `docs/agents/project-workflow.md` § godot-ai addon). Upgrades *construction* over godot-mcp's run/edit baseline — but the two heavily overlap (scene/node/script), so **prefer one writer per editor instance** across them.
