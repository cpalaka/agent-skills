...
**Tool naming in this guide.** Tools are named bare, under their server — godot-mcp's `godot_docs`, godot-ai's `logs_read`, minimal-godot's `get_diagnostics`. How your host spells them is a host detail: Claude Code exposes each as `mcp__<server>__<tool>`, Codex exposes them under the bare server name. The server a tool belongs to, and the role that server plays, are not host details — they are what this guide is about.

## The MCP servers

- **godot-mcp** (v4.1.0 — templates scaffold this pin; existing projects may still be on 3.6.1, check `tools/mcp/package.json`) — drives the editor via WebSocket bridge (port 6550). Tools (all named `godot_*`). On **4.1.0** (surface read from `dist/tools/` on 2026-08-08): `godot_scene`, `godot_scene3d`, `godot_resource`, `godot_project`, `godot_input`, `godot_runtime_state`, `godot_docs`, `godot_profiler`, split read/edit pairs `godot_node_read`/`godot_node_edit`, `godot_editor_read`/`godot_editor_edit`, `godot_animation_read`/`godot_animation_edit`, `godot_tilemap_read`/`godot_tilemap_edit`, `godot_gridmap_read`/`godot_gridmap_edit`, plus new `godot_exec`, `godot_game_time`, `godot_validate_meshes`. On **3.6.1** the same surfaces are unsplit (`godot_node`, `godot_editor`, `godot_animation`, `godot_tilemap`, `godot_gridmap`). **READ/TEST role only — never its write ops** (see the matrix below). **Version history of the pin:** v3.6.1 was held through July 2026 on a June-2026 field scan showing v4.0 had dropped the authoring verbs — a blocker only while godot-mcp was a candidate writer. That hold is obsolete twice over: godot-ai owns all writes in this stack, and 4.1.0 ships `*_edit` verbs anyway. New projects take 4.1.0; there is no reason to seek out 3.6.1.
- **minimal-godot-mcp** (its project-scope server key is `godot`, so its tools appear under the server name `godot` — grepping the MCP config for "minimal-godot" finds nothing) — filesystem-based. Tools: GDScript diagnostics (`get_diagnostics`), runtime console output capture (`get_console_output`).
- **godot-ai** (`hi-godot/godot-ai` v4.1.0, vendored at `addons/godot_ai/`, **tracked in git** — it self-updates in-editor, gotcha #116; added 2026-05-30, last bumped 2026-09-19) — construction-grade editor MCP. **v4 needs Godot 4.7+ (within the 4.x line) and `uv` on PATH**; the plugin auto-starts its Python server and connects over **WebSocket**. Register a client through the **Godot AI** dock's **Configure** button, which writes a generated `godot-ai attach` stdio entry carrying the matching version, ports and tool-domain filters — **do not hand-write a bare HTTP MCP entry**: `http://127.0.0.1:8000/mcp` was the v3 shape, and v3→v4 is a migration (`docs/v4-migration.md` in the addon repo). Catalog at 4.1.0: **41 non-core tools across 27 domains**, plus 4 core and 1 always-on, read from `addons/godot_ai/tool_catalog.gd` on 2026-09-19 — the same domain and tool counts 4.0.2 carried, so a 4.0.2→4.1.0 bump moves no tool surface. The surface listed here was diffed at **v3.2.4** against the 3.1.3 tree on 2026-09-02 and is kept as that version's record (new since 3.1.3: `gridmap_manage`, `csg_manage`, `custom_manage` — the last invokes tools other addons register via the 3.2.0 custom-tool registry; nothing here registers one): `scene_*`/`node_*`/`script_*`, `signal_manage`, `material`/`particle`/`camera`/`animation`/`ui_manage`, `project_run`, `editor_screenshot`, `logs_read`, `game_manage` (v2.7.x: **raw** key/mouse/gamepad input injection — `input_key`/`input_mouse`/`input_gamepad`, which drive any *mapped* action — plus runtime node-state read `get_node_info`/`get_scene_tree` and named-action-state read `input_state`; verified live on 2.7.5 / Godot 4.7). Enabled as a project plugin. Since 3.2.0 the dock's client registration scope is an EditorSettings enum (`user` default → the stdio entry lives in the host's user-scope config, not `.mcp.json`; see the contract, `docs/agents/project-workflow.md` § godot-ai addon). Upgrades *construction* over godot-mcp's run/edit baseline — but the two heavily overlap (scene/node/script), so **prefer one writer per editor instance** across them.
...
- **`godot-mcp-clean` does NOT reap godot-ai's server.** It only kills `node ...godot-mcp` processes. godot-ai runs a managed Python/uv server (and possibly `uvx mcp-proxy`); after an unclean exit, find/kill via `lsof -nP -iTCP:8000` / `:9500` — read the dock's *resolved* ports rather than assuming those defaults, which the dock will walk off when they are taken. Avoid `editor_manage(op=quit)` to close the editor — teardown can race editor exit (godot-ai issue #401).
...
- **`godot_input` does relative mouse-look, not cursor positioning.** On 4.1.0 a `{look: [dx, dy]}` entry drives FPS-camera `_input` handlers; it still cannot move the pointer or click a screen position. (3.6.1 has no mouse support at all; a 4.1.0 server over an older *addon* prints `WARNING: relative mouse-look entries were IGNORED` — read the tool result, the call still "succeeds".) godot-ai's `game_manage input_mouse` *does* inject raw mouse button/motion events (for handlers reading `event.position`), **but** a handler reading `get_global_mouse_position()` still won't see them — that reads the OS cursor, not the injected event (godot-gotchas #32, archived). So for cursor-**aim** mechanics (an aim indicator, a node reading `get_global_mouse_position()`) there's still no clean injection route: move the real OS mouse and verify with a `source="game"` screenshot, hold the action stationary (see next bullet), or temporarily drive the value from a script. (`godot_runtime_state` is likewise `action`-discriminated: `digest` | `watch_start` | `watch_collect` | `watch_stop` — not `operation`/`get_property`.)
