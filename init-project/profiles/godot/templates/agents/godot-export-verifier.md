...
<!-- TEMPLATE NOTE (delete after customizing): this file is stamped by init-project's godot
     ...
     guesses. A full customization pins the preset table, the build-kind output layout, the
     export-wrapper contract, and a smoke subset. -->
...
- **MCP-tooling projects: shipping presets MUST route through an autoload-stripping wrapper —
  ...
  the project's wrapper command here (reference shape:
...
2. **Headless Godot MUST run with the Claude Code sandbox disabled** (the
...
- For a shipping preset in an MCP-tooling project, also confirm the strip worked:
  scan the `.pck` contents (or boot the build if the platform allows) for the MCP scripts /
  autoload errors — a clean export log proves nothing.
...
- If an export fails in a way that smells like an engine quirk, surface it but don't
  speculate on the fix — check `docs/godot-gotchas.md`, and, if the `godot-gotchas` skill is
  installed (its directory exists under `~/.claude/skills` or `~/.agents/skills`), its catalog
  too; otherwise skip that lookup. Or read the engine side through godot-ai
