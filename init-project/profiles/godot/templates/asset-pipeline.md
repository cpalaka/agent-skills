...
**Recommended default for real projects:** Blender working tree lives *outside* the Godot project, in a sibling location. Only exported assets (`.glb` / `.gltf`) cross the boundary. `{{WORKSPACE_ROOT}}` below is the directory that holds both trees — init-project asks for it once and stamps it in.
...
```python
# in {{WORKSPACE_ROOT}}/blender/<game-name>/export.py
import bpy
import os
...
The godot-mcp server's `godot_resource get_info` tool (Claude Code tool prefix `mcp__godot-mcp__`; Codex names the same server `godot-mcp`) only reports a subset of `StandardMaterial3D` properties — for full introspection, attach a GDScript and `print()` the values at runtime.
