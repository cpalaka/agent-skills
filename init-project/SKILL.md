...
**2. Enable external @imports (load-bearing — see ADR-0001).**
...
`--add-dir ~/.claude/chunks` — `--add-dir` changes nothing either way). The engine records this
in the handoff (step 8); it does **not** auto-edit `~/.claude.json` unless the user explicitly
opts in.
...
the user supplies for NAME; ask once per distinct token. Templates are *copied + parity-tracked*
(unlike chunks); their source of truth is the Profile asset, kept aligned by a parity check,
never hand-merged.
...
expand the imports; (c) any fresh-clone rehydrate command from step 6; (d) anything the Profile
