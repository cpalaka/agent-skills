...
5. **Deviations are report items.** Any place you departed from the spec, say so
   explicitly and why.
6. **On a Godot project, consult the gotcha catalog before you write code.** Run
   `~/.claude/skills/godot-gotchas/scripts/lookup.sh <words>` on the phase's APIs and
   symptoms, read the bodies it names, and end your phase report with that skill's Outcome
   line — `gotchas: consulted #a #b; applied #a`, or `; applied none`, or
   `gotchas: consulted none — <why>`. The three forms and where they go are defined in the
   skill's `SKILL.md`; a Godot phase whose report carries no line counts as never looked.
