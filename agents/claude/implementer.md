...
6. **On a Godot project, consult the gotcha catalog before you write code.** Gated on
   ...
   skill's Outcome line — `gotchas: consulted #a #b; applied #a`,
   `gotchas: consulted #a #b; applied none`, or `gotchas: consulted none — <why>`. Those three
   forms are defined, and defined only, in the skill's `SKILL.md`; restated here so a seat can
   write one without loading the router. A Godot phase whose report carries no line counts as
   never looked.
