...
const rawJudged = await pipeline(
  ...
  (generated, idx) => {
    ...
    if (!generated) return { index: idx, name: c.name, score: null, votesSent: 0, votesReturned: 0, dropped: 0, errored: [], generationFailed: true, stageThrew: false }
    return parallel(JUDGES.map(j => () => agent(`judge ${c.name} as ${j.persona}: ${j.rubric}`, { model: 'opus', schema: JUDGE_SCHEMA })))
      .then(js => {
        ...
        return { index: idx, name: c.name, score, votesSent: js.length, votesReturned: valid.length, dropped, errored, generationFailed: false, stageThrew: false }
      })
  }
)
