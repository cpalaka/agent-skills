...
A copy of the original mix lands in that folder too, under its own filename, so the folder is self-contained — the source often lives on a network share or an external drive that will not be mounted next time the Ableton set is opened, and stems with no mix beside them cannot be A/B'd. The copy is taken from the decode-verified staged file, not re-read from the source, and only on a run that passed verification.
...
- Network mounts return corrupt reads of intact files, and the OS caches the bad copy — so it hashes stable for minutes while the file's own mtime never moves. The script re-reads past the cache and gates on decoded length, up to three times, which handles this silently.
- When it gives up anyway, the question is whether the mount or the file is at fault, and a hash cannot tell you: **copy the file twice, minutes apart, and byte-diff the copies.** Regions that agree across independent reads are what the server actually holds, so damage there is in the file itself; regions that differ are the mount. A file whose head is stable and structurally broken is unrecoverable — re-reading it forever will not help. Observed once: a 103 MB FLAC with a valid header, a bogus metadata block chain, and zero frame-sync codes in 20 MB, against 2229 in a healthy file.
